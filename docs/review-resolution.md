...
本清单合并多轮报告中的重复项。首个预发布版本为 0.1.0-beta.1。

| # | 问题 | 当前处理与验证 |
| --- | --- | --- |
| 1 | 调度反复复制、排序队列，完成时扫描 Scope | 改用按优先级、组和 Pool/Session 通道组织的索引堆；入队缓存 groupKey；完成时直接删除对应索引。8000 项选择计数回归和真实 Worker 基准覆盖。 |
...
| 4 | 字符串、普通数组、scratch 可绕过预算 | 输入输出采用含元数据计费的 Packet；普通缓存使用 dataByteLength；scratch arena 拒绝超额分配。普通 JS/native 分配由应用负责估算和约束，契约明确其管理范围。 |
| 5 | 遍历先分配整层再拒绝、原始值占对象配额 | 数组/集合提前检查尺寸，逐项限制对象、边和工作栈；数字不占对象数。20 万数字数组往返、超宽数组拒绝前零 getter 调用覆盖。 |
| 6 | Worker/主线程重复遍历结果，收到后才验证额度 | Host 编码并检查后才发送；主线程只检查封装字节，首次 value 访问才解码。接收侧反序列化先于 Runtime 校验；契约要求运行可信 Worker，自定义 endpoint 遵守发送前校验规则。 |
| 7 | progress 无大小限制和背压 | 4 KiB 小型控制消息、单条在途 ACK、只保留最新快照；Node 和三个浏览器验证。 |
| 8 | 不结束的异步 prepare 占额度、阻塞 dispose | API 改为同步构造；Promise/thenable 立即拒绝并归还额度；异步工作在 Worker 内执行。同步回调需及时返回以保持主线程响应，已启动的外部副作用由回调负责处理。 |
| 9 | terminate 失败永久隔离 | 保留真实额度，提供 retryTermination 和隔离诊断；故障恢复后重试验证无双重释放。 |
| 10 | 忘记 release 的结果驻留 | consumeResult 保证 finally 释放，discardResult 支持仅等 settled；maxResultLeases 限制积累。保留的租约采用显式所有权与确定性释放。 |
...
| 20 | Session 类/WASM 资源的存储与清理 | setResource 独立声明计账并注册异步 disposer；清理失败保留资源/Worker 供重试。硬终止路径的外部资源由应用恢复策略清理。 |
| 21 | 启动中取消绕过执行截止时间 | 独立物理 phase；启动不应答后取消仍按执行截止时间回收的测试覆盖。 |
| 22 | hello 同步失败后 Session 绑定已关闭槽位 | spawn 失败抛错、绑定前检查状态；后续任务通过失败路径及时结束。 |
...
调度基准使用 1 个预热 Node Worker、8 个 interactive 组、一次性提交，每档 3 次。此次 500/1000/2000/8000 项批次总耗时中位数为 19.3/24.0/47.6/148.5 ms；最大定时器延迟分别为 1.9/1.4/2.2/12.3 ms。批次墙钟时间、定时器延迟和 CPU profile 时间各有测量口径，应在相同口径下进行比较。

缓存 key 构造每档 100 万次、预热后 3 次，此次 JSON tuple 中位数 69.0 ms，命名空间前缀 7.9 ms；测量范围为 key 构造；缓存查找、验证和驱逐开销需另行测量。

## 发布边界

beta 使用 npm 的 beta dist-tag 与 GitHub prerelease。发布前验证最终内容，版本 tag 必须对应发布提交；发布后核对注册表版本、下载包与 GitHub Release。包内不携带原始基准 JSON。

资源预算控制协议数据、已声明暂存和常驻资源。应用负责管理操作系统句柄及外部引用，并结合运行环境监测 JS 堆和 RSS；各层职责见资源契约。
